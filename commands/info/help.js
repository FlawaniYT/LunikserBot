...
module.exports = {
    ...
    aliases: ["h"],
    category: "info",
    description: "Zwraca wszystkie polecenia lub informacje o jednym poleceniu",
    usage: "[polecenie | Alias]",
...
function getCMD(client, message, input) {
    ...
    let info = `Nie znaleziono informacji dla polecenia **${input.toLowerCase()}**`;
    ...
    if (cmd.name) info = `**Nazwa polecenia**: ${cmd.name}`;
    if (cmd.aliases) info += `\n**Aliasy**: ${cmd.aliases.map(a => `\`${a}\``).join(", ")}`;
    if (cmd.description) info += `\n**Opis**: ${cmd.description}`;
    if (cmd.usage) {
        info += `\n**Stosowanie**: ${cmd.usage}`;
